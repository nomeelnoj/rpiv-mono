...
- Per-executor fallback — when a `perExecutor` override advisor fails hard (auth failure, an `error` stop reason such as a provider content filter, or a thrown exception), the side-call retries once with the default advisor. The fallback only runs when a default is configured and resolves to a different model than the override; a user abort or an empty response is treated as terminal and never triggers a retry. The fallback result records `fallbackFrom` and `fallbackReason` in its details for observability.
