...
### Added
- `perExecutorGuidance` config field — override the injected `advisor` tool guidance (`promptSnippet` / `promptGuidelines`) per executor. List `{ models: string[], guidance }` entries in `~/.config/rpiv-advisor/advisor.json`; group several model keys under one block so variant families share one policy. Resolution is field-level and three-tiered (matched block → global `guidance` → built-in defaults), so a partial override inherits the remaining fields. The active override is re-applied on `model_select` and at agent start, and the refresh guard re-registers when either the description or the resolved guidance changes. Motivating case: telling eager instruction-followers to never call the advisor as a first step while keeping it available on complex work.
...
- Advisor chain walking — `advisor()` accepts optional `depth` and `target` to walk a chain of advisors implied by the `perExecutor` table (each model's `advisor` entry is the next hop). `depth: N` walks up to N tiers; `target: "<name|provider:id>"` walks until a matching model is reached (target wins, capped by depth). Each tier sees the executor branch plus prior advisor responses; the final tier's response is returned. Cycles and dead-ends terminate gracefully and return the deepest completed response. `advisor()` with no arguments is unchanged (single hop). The tool description updates on model switch (and after `/advisor` route edits) to show the chain available from the current executor.
